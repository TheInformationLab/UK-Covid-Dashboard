...
const SingleSelectMenu = (props) => {
  ...
  return (
    <div className="space-y-1" style={{width: props.width}}>
      <label id="listbox-label" className="block text-sm leading-5 font-medium text-gray-700">
        {props.label}
      </label>
      <div className="relative">
        <span className="inline-block w-full rounded-md shadow-sm">
          <button type="button" onMouseDown={() => setOpen(true)} onFocus={() => setOpen(true)} aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label" className="cursor-default relative w-full rounded-md border border-gray-300 bg-white pl-3 pr-10 py-2 text-left focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition ease-in-out duration-150 sm:text-sm sm:leading-5">
        ...
        <div ref={wrapperRef} className={"absolute mt-1 w-full rounded-md bg-white shadow-lg " + (!open ? 'hidden' : '')}>
            ...
            {props.options.map((option, idx) => {
              return (
                <li id={"listbox-item-" + idx}
                  ...
                  role="option"
                  className={"cursor-default select-none relative px-2 py-2 pl-8 pr-4 " + (hover === idx ? 'text-white bg-indigo-600' : 'text-gray-900')}
                >
                  <span className={"font-normal block truncate " + (option.value === props.value ? 'font-semibold' : '')}>
                    {option.label}
                  </span>
                  <span className={"absolute inset-y-0 left-0 flex items-center ml-2 pl-1.5 " + (option.value !== props.value ? 'hidden ' : '') + (hover === idx ? 'text-white' : 'text-indigo-600')}>
